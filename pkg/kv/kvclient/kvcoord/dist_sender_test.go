...
func TestDistSenderComputeNetworkCost(t *testing.T) {
	...
	costTable := `{"regionPairs": [
		{"fromRegion": "us-east1", "toRegion": "us-east1", "cost": 0},
		...
		{"fromRegion": "eu-central1", "toRegion": "eu-central1", "cost": 0},
		...
		{"fromRegion": "asia-southeast1", "toRegion": "asia-southeast1", "cost": 0}
	]}`
	require.NoError(t, tenantcostmodel.RegionalCostMultiplierTableSetting.Validate(nil, costTable))
	tenantcostmodel.RegionalCostMultiplierTableSetting.Override(ctx, &st.SV, costTable)
	...
	for _, tc := range []struct {
		...
		// TODO(jeffswenson): audit dist_sender to ensure this is never actually needed
		//{
