...
func TestDeleteRangeTombstone(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		const (
			...
			rangeTombstonePredicate               = "range-tombstone"
			pointTombstonePredicate               = "point-tobmstone"
		)
...
func checkPredicateDeleteRange(t *testing.T, engine storage.Reader, rKeyInfo storage.MVCCRangeKey) {
	// TODO(jeffswenson): if there is a tombstone, ensure it is not in addition
	// to a pre-existing range key.
...
// TODO(jeffswenson): test what happens when you delete this range that
// contains a range tombstone. There should be one point tombstone for "d".
//  2 [a--------d)
//  1     b     d
//     a  b  c  d

// TODO(jeffswenson): write a simple test that deletes a range twice and ensure
// that there is one point tombstone for each point value.
//  3
//  2  a
//  1     b     d
//     a  b  c  d

// TODO(jeffswenson): write a simple test that deletes a range containing an intent.
//  3
//  2  a    <c>
//  1     b     d
//     a  b  c  d
