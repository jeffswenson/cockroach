...
type Registry struct {
	...
	//
	// The uri ispassed to the registry at construction time. The baseURI but it
	// is expected to be of the form "scheme://<bucket>/roachprod/<version>". So
	// a full metadata path looks like:
...
func NewRegistry(ctx context.Context, uri url.URL) (*Registry, error) {
	supportedSchemes := map[string]bool{"gs": true, "s3": true, "azure": true, "nodelocal": true}
	...
	var storage cloud.ExternalStorage
	var err error
	if uri.Scheme != "nodelocal" {
		...
		storage = nodelocal.TestingMakeNodelocalStorage(root, cluster.MakeTestingClusterSettings(), cloudpb.ExternalStorage{})
	}
...
func (r *Registry) listFixtures(
	...
	err := r.storage.List(ctx, kindPrefix /*delimiter*/, "", func(found string) error {
		// TODO(jeffswenson): does this need to change behavior base on cloud vs
		// local?
		...
		metadata := FixtureMetadata{}
		if err := metadata.Unmarshal(json); err != nil {
			return err
		}
