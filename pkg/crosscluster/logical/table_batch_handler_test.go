...
func newTestTableHandler(
	...
	handler, err := newTableHandler(context.Background(), descID, s.InternalDB().(descs.DB), s.Codec(), sd, s.LeaseManager().(*lease.Manager), s.ClusterSettings(), 0)
...
func TestTableHandlerFastPath(t *testing.T) {
	...
	// Create a test runner for executing SQL statements
	...
	// TODO(jeffswenson): how do I detect that the fast path is working?  Maybe
	// it should be part of the stats?
	...
	// TODO test all the cases where the fast path works
...
func TestTableHandlerExhaustive(t *testing.T) {
	...
	expectedRows := map[string]expected{}
	for _, tc := range testCases {
		expect := expected{
			tc: tc,
		}
		...
		case tc.winLww && tc.replicationType != replicationTypeDelete:
			expect.expected = getNewValue(tc)
		case !tc.winLww:
			expect.expected = getLocalValue(tc)
		}
		expectedRows[fmt.Sprintf("%d", tc.id)] = expect
	}

	results := runner.QueryStr(t, `SELECT id, value FROM test_table`)
	for _, row := range results {
		expect := expectedRows[row[0]]
		expect.found = row[1]
		expectedRows[row[0]] = expect
	}

	for _, expect := range expectedRows {
		require.Equal(t, expect.expected, expect.found, "test case id:%d name:%s", expect.tc.id, testCaseName(expect.tc))
	}
}
