...
// BenchmarkDeriveLocks measures the performance of lock synthesis and
// topological sorting across different transaction sizes.
func BenchmarkDeriveLocks(b *testing.B) {
	tableID := descpb.ID(100)

	pkMixin, err := tableMixin(tableID)
	if err != nil {
		b.Fatal(err)
	}
