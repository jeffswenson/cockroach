...
// Package txnlock synthesizes replication locks for logical data
// replication (LDR). LDR replicates row-level changes from a source
// cluster to a destination cluster by consuming rangefeed events.
// Within a single transaction, lock synthesis derives a set of locks
// from each row's constraints, then topologically sorts the rows to
// produce a valid intra-transaction apply order. Across transactions,
// the scheduler uses the synthesized locks to identify dependencies:
// two transactions conflict when they hold locks with the same lock_id
// and at least one is a write, and conflicting transactions must be
// applied in commit-timestamp order.
//
// # Replication Locks
//
