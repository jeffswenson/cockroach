...
func TestSQLRowReader(t *testing.T) {
	...
	dbSource.Exec(t, "INSERT INTO tab VALUES (1, 'one'), (2, 'two'), (3, 'three')")
	dbSource.Exec(t, "DELETE FROM tab WHERE pk = 2")
	now := s.Clock().Now()
	...
	testRows := []tree.Datums{
		...
		{tree.NewDInt(1), tree.NewDString("two")},   // Deleted row
		{tree.NewDInt(2), tree.NewDString("three")}, // Existing row
		{tree.NewDInt(3), tree.NewDString("four")},  // Never existed
	}
	primaryKeys := []int{0, 1, 2, 3}
