...
func newFixtureRegistry(ctx context.Context, t test.Test, c cluster.Cluster) *blobfixture.Registry {
	...
	switch c.Cloud() {
	case spec.AWS:
		uri = "s3://cockroach-fixtures-us-east-2?AUTH=implicit"
	case spec.GCE:
		uri = "gs://cockroach-fixtures-us-east1?AUTH=implicit"
	case spec.Local:
		uri = "nodelocal://1/fixtures"
	default:
	...
	url.Path = path.Join(url.Path, "roachtest/v25.1")

	registry, err := blobfixture.NewRegistry(ctx, *url) 
	require.NoError(t, err)
...
func registerBackupFixtures(r registry.Registry) {
	specs := []backupFixtureSpecs{
		...
		},
		{
			...
			clouds:  []spec.Cloud{spec.AWS, spec.GCE},
			skip:    "skipped until nightly fixture tests are stable",
		},
		{
	...
	for _, bf := range specs {
		...
		r.Add(registry.TestSpec{
			...
			CompatibleClouds:  registry.Clouds(spec.GCE, spec.AWS, spec.Local),
			Suites:            bf.suites,
			Skip:              bf.skip,
